...
"""
   NonlinearResistor(; name, Ga, Gb, Ve)
...
| `i`         |                          | A  | 
"""
@component function NonlinearResistor(; name, Ga=nothing, Gb=nothing, Ve=nothing)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (Ga::Float64 = Ga), [description = "Conductance in inner voltage range"])
  append!(__params, @parameters (Gb::Float64 = Gb), [description = "Conductance in outer voltage range"])
  append!(__params, @parameters (Ve::Float64 = Ve), [description = "Inner voltage range limit"])
  ...
  push!(__eqs, i ~ ifelse((v < -Ve), (Gb * (v + Ve) - Ga * Ve), ifelse((v > Ve), (Gb * (v - Ve) + Ga * Ve), Ga * v)))

  # Return completely constructed ODESystem
  return ODESystem(__eqs, t, __vars, __params; systems=__systems, defaults=__defaults, name, initialization_eqs=__initialization_eqs)
end
export NonlinearResistor

Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(NonlinearResistor)) = print(io,
